        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 lg:py-2 space-y-6">
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <div class="flex w-full justify-center">
                    <form action="{{ route('santris.update', $santri->id) }}" method="POST" class="w-3/4">
                        @csrf
                        @method('put')
                        <div class="pb-3">
                            ...
                            <input type="text" id="nama" class="w-full border border-gray-200 rounded-lg"
                                name="nama" value="{{ old('nama') ?? $santri->nama }}" required>
                        </div>
                        <div class="pb-3">
                            ...
                            <input type="date" id="tgl_lahir" class="w-full border border-gray-200 rounded-lg"
                                name="tgl_lahir" value="{{ old('tgl_lahir') ?? $santri->tgl_lahir }}" required>
                        </div>
                        <div class="pb-3">
                            ...
                            <input type="text" id="lahir" class="w-full border border-gray-200 rounded-lg"
                                name="lahir" value="{{ old('lahir') ?? $santri->lahir }}" required>
                        </div>
                        <div class="pb-3">
                            ...
                            <input type="text" id="no_hp" class="w-full border border-gray-200 rounded-lg"
                                name="no_hp" value="{{ old('no_hp') ?? $santri->no_hp }}" required>
                        </div>
                        <div class="pb-3">
                            <label for="alamat" class="w-full text-sm font-semibold text-gray-800">Alamat</label>
                            <textarea id="alamat" class="w-full border border-gray-200 rounded-lg" name="alamat" required>{{ old('alamat') ?? $santri->alamat }}</textarea>
                        </div>
